...
- `decorations: false` + `titleBarStyle: "overlay"` gives Arc-style frameless window on macOS; traffic lights float at top-left
- Tab bar must have `pl-[72px]` left padding to clear the macOS traffic light cluster (~72px)
...
- [ ] `src/types/panel.ts`:
  ```ts
  export type PanelLeaf = { type: "leaf"; id: string; pluginId: string | null };
