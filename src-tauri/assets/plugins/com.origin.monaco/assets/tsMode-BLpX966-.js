      ...
      this._client ||
        (this._client = (async () => (
          (this._worker = M({
            ...
            createWorker: () =>
              new Worker(
                new URL("/assets/ts.worker-BH9nVgjN.js", import.meta.url),
                { type: "module" },
              ),
            label: this._modeId,
            keepIdleModels: !0,
...
function A(c, e, t = 0) {
  ...
  if (((o += c.messageText), t++, c.next))
    for (const i of c.next) o += A(i, e, t);
  return o;
}
function _(c) {
...
class V extends y {
  constructor(e, t, o, i) {
    ...
    const l = (r) => {
        ...
        const u = () => {
          ...
          h
            ? r.isAttachedToEditor() && this._doValidate(r)
            : this._doValidate(r);
        };
        let d;
      ...
      p = (r) => {
        ...
        this._listener[u] &&
          (this._listener[u].dispose(), delete this._listener[u]);
      };
    (this._disposables.push(w.onDidCreateModel((r) => l(r))),
      this._disposables.push(w.onWillDisposeModel(p)),
  ...
  async _doValidate(e) {
    ...
    (i || o.push(t.getSyntacticDiagnostics(e.uri.toString())),
    ...
    const r = n
      ...
      u = r
        ...
        .reduce((d, g) => g.concat(d), [])
        .map((d) => (d.file ? F.parse(d.file.fileName) : null));
    (await this._libFiles.fetchLibFilesIfNecessary(u),
      !e.isDisposed() &&
        w.setModelMarkers(
          e,
          this._selector,
  ...
  _convertDiagnostics(e, t) {
    const o = t.start || 0,
      ...
      { lineNumber: n, column: r } = e.getPositionAt(o + i),
      u = [];
    return (
      t.reportsUnnecessary && u.push(P.Unnecessary),
      t.reportsDeprecated && u.push(P.Deprecated),
      {
  ...
  _convertRelatedInformation(e, t) {
    ...
    return (
      t.forEach((i) => {
        let l = e;
        if (
          (i.file && (l = this._libFiles.getOrCreateModel(i.file.fileName)), !l)
        )
          return;
        const p = i.start || 0,
          n = i.length || 1,
          { lineNumber: r, column: u } = l.getPositionAt(p),
          { lineNumber: d, column: g } = l.getPositionAt(p + n);
        o.push({
          resource: l.uri,
          startLineNumber: r,
          startColumn: u,
...
class C extends y {
  ...
  async provideCompletionItems(e, t, o, i) {
    const l = e.getWordUntilPosition(t),
      p = new O(t.lineNumber, l.startColumn, t.lineNumber, l.endColumn),
      n = e.uri,
      r = e.getOffsetAt(t),
      ...
      : {
          suggestions: d.entries.map((b) => {
            ...
            if (b.replacementSpan) {
              const x = e.getPositionAt(b.replacementSpan.start),
                D = e.getPositionAt(
            ...
            const k = [];
            return (
              b.kindModifiers !== void 0 &&
                b.kindModifiers.indexOf("deprecated") !== -1 &&
                k.push(f.CompletionItemTag.Deprecated),
              {
                uri: n,
                position: t,
                offset: r,
  ...
  async resolveCompletionItem(e, t) {
    const o = e,
      ...
      p = o.offset,
      r = await (
        await this._worker(i)
      ).getCompletionEntryDetails(i.toString(), p, o.label);
    return r
      ? {
          uri: i,
          position: l,
...
function N(c) {
  ...
  if (c.name === "param" && c.text) {
    const [t, ...o] = c.text;
    ((e += `\`${t.text}\``),
      o.length > 0 && (e += ` — ${o.map((i) => i.text).join(" ")}`));
  } else
    Array.isArray(c.text)
      ? (e += ` — ${c.text.map((t) => t.text).join(" ")}`)
      : c.text && (e += ` — ${c.text}`);
  return e;
}
class I extends y {
  ...
  async provideSignatureHelp(e, t, o, i) {
    ...
    return (
      r.items.forEach((d) => {
        ...
        ((g.documentation = { value: _(d.documentation) }),
          (g.label += _(d.prefixDisplayParts)),
          d.parameters.forEach((b, h, k) => {
            const x = _(b.displayParts),
              D = { label: x, documentation: { value: _(b.documentation) } };
            ((g.label += x),
              g.parameters.push(D),
              h < k.length - 1 && (g.label += _(d.separatorDisplayParts)));
          }),
          (g.label += _(d.suffixDisplayParts)),
          u.signatures.push(g));
      }),
      { value: u, dispose() {} }
    );
...
class W extends y {
  async provideHover(e, t, o) {
    const i = e.uri,
      l = e.getOffsetAt(t),
      p = await this._worker(i);
    if (e.isDisposed()) return;
    const n = await p.getQuickInfoAtPosition(i.toString(), l);
    if (!n || e.isDisposed()) return;
    const r = _(n.documentation),
      u = n.tags
        ? n.tags.map((g) => N(g)).join(`  
...
class j extends y {
  async provideDocumentHighlights(e, t, o) {
    ...
    if (!(!n || e.isDisposed()))
      return n.flatMap((r) =>
        r.highlightSpans.map((u) => ({
          range: this._textSpanToRange(e, u.textSpan),
          kind:
            u.kind === "writtenReference"
              ? f.DocumentHighlightKind.Write
              : f.DocumentHighlightKind.Text,
        })),
      );
  }
}
...
class z extends S {
  ...
  async provideDocumentRangeFormattingEdits(e, t, o, i) {
    const l = e.uri,
      ...
      n = e.getOffsetAt({ lineNumber: t.endLineNumber, column: t.endColumn }),
...
class J extends S {
  async provideCodeActions(e, t, o, i) {
    const l = e.uri,
      ...
      n = e.getOffsetAt({ lineNumber: t.endLineNumber, column: t.endColumn }),
      r = S._convertOptions(e.getOptions()),
      u = o.markers
        .filter((h) => h.code)
        .map((h) => h.code)
        .map(Number),
      d = await this._worker(l);
    if (e.isDisposed()) return;
    ...
    return !g || e.isDisposed()
      ...
      : {
          actions: g
            ...
            .map((h) => this._tsCodeFixActionToMonacoCodeAction(e, o, h)),
          dispose: () => {},
        };
  }
...
class Q extends y {
  ...
  async provideRenameEdits(e, t, o, i) {
    const l = e.uri,
      ...
      r = await this._worker(l);
    if (e.isDisposed()) return;
    const u = await r.getRenameInfo(p, n, { allowRenameOfImportPath: !1 });
    if (u.canRename === !1)
      return { edits: [], rejectReason: u.localizedErrorMessage };
    if (u.fileToRename !== void 0)
      throw new Error("Renaming files is not supported.");
    const d = await r.findRenameLocations(p, n, !1, !1, !1);
    ...
    for (const b of d) {
      ...
      if (h)
        g.push({
          resource: h.uri,
          versionId: void 0,
          textEdit: { range: this._textSpanToRange(h, b.textSpan), text: o },
        });
...
function R(c, e) {
  const t = [],
    o = new E(e, c),
    i = (...n) => o.getLanguageServiceWorker(...n),
    l = new H(i);
  function p() {
    const { modeConfiguration: n } = c;
    (X(t),
      n.completionItems &&
        t.push(f.registerCompletionItemProvider(e, new C(i))),
      n.signatureHelp && t.push(f.registerSignatureHelpProvider(e, new I(i))),
      n.hovers && t.push(f.registerHoverProvider(e, new W(i))),
      n.documentHighlights &&
        t.push(f.registerDocumentHighlightProvider(e, new j(i))),
      n.definitions && t.push(f.registerDefinitionProvider(e, new B(l, i))),
      n.references && t.push(f.registerReferenceProvider(e, new U(l, i))),
      n.documentSymbols &&
        t.push(f.registerDocumentSymbolProvider(e, new $(i))),
      n.rename && t.push(f.registerRenameProvider(e, new Q(l, i))),
      n.documentRangeFormattingEdits &&
        t.push(f.registerDocumentRangeFormattingEditProvider(e, new z(i))),
      n.onTypeFormattingEdits &&
        t.push(f.registerOnTypeFormattingEditProvider(e, new G(i))),
      n.codeActions && t.push(f.registerCodeActionProvider(e, new J(i))),
      n.inlayHints && t.push(f.registerInlayHintsProvider(e, new q(i))),
